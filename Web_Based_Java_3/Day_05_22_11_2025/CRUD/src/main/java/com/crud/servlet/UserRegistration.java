...
public class UserRegistration extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        // Join the array elements into a single comma-separated string required by MySQL SET
        String hobbiesString = null;
        if (hobbiesArray != null && hobbiesArray.length > 0) {
            hobbiesString = String.join(",", hobbiesArray);
        }
		...
		//boolean status=lservice.addUser(uname,pass);
		boolean status=lservice.registerUser(name,email,gender,hobbiesString,city);
		
		if(status)
		{
			
			
			out.println("Registration succesfully, Log in :)");
