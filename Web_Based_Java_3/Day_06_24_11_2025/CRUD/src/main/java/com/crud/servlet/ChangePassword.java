package com.crud.servlet;

import java.io.IOException;
import java.io.PrintWriter;

import jakarta.servlet.RequestDispatcher;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.HttpServlet;
import jakarta.servlet.http.HttpServletRequest;
import jakarta.servlet.http.HttpServletResponse;

import com.crud.service.*;


@WebServlet("/changed")
public class ChangePassword extends HttpServlet {
	private static final long serialVersionUID = 1L;



	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		
		response.setContentType("text/html");
		
		PrintWriter out = response.getWriter();
		
		String uname= request.getParameter("uname");
		String oldp= request.getParameter("oldpass");
		String newp= request.getParameter("newpass");
		
		LoginService lservice= new LoginServiceImpl();
		
		boolean status=lservice.changePass(uname,oldp,newp);
		
		if(status)
		{
			
			
			out.println("Password Changed , Try Logging in Again :)");
			
			RequestDispatcher rd = request.getRequestDispatcher("LoginPage.html");
			rd.include(request, response);
		}
		else {
			
			out.println("Password Not Changed , Enter Correct Details");
			RequestDispatcher rd = request.getRequestDispatcher("ChangePassword.html");
			rd.include(request, response);
			
		}
		
		
		
		
		
	}

}
